...
/*************************************************
	using exp function
*************************************************/
class GaussianRangeFilter_EXP_64f_InvokerAVX512 : public cv::ParallelLoopBody
{
public:
	GaussianRangeFilter_EXP_64f_InvokerAVX512(Mat& _dest, const Mat& _temp, const int _radiusH, const int _radiusV, const int _maxk, const int* _space_ofs, const double _sigma_range) :
...
class GaussianRangeFilter_EXP_32f_InvokerAVX512 : public cv::ParallelLoopBody
{
public:
	GaussianRangeFilter_EXP_32f_InvokerAVX512(Mat& _dest, const Mat& _temp, const int _radiusH, const int _radiusV, const int _maxk, const int* _space_ofs, const float _sigma_range) :
		temp(&_temp), dest(&_dest), radiusH(_radiusH), radiusV(_radiusV), maxk(_maxk), space_ofs(_space_ofs), sigma_range(_sigma_range)
	{
	}
...
class GaussianRangeFilter_EXP_8u_InvokerAVX512 : public cv::ParallelLoopBody
{
public:
	GaussianRangeFilter_EXP_8u_InvokerAVX512(Mat& _dest, const Mat& _temp, const int _radiusH, const int _radiusV, const int _maxk, const int* _space_ofs, const float _sigma_range) :
		temp(&_temp), dest(&_dest), radiusH(_radiusH), radiusV(_radiusV), maxk(_maxk), space_ofs(_space_ofs), sigma_range(_sigma_range)
	{
	}
				...
				if (haveAVX512)
				{
...
#if __GRF_PREVENTION__
					static const __m512 exp_min_arg = _mm512_set1_ps(EXP_ARGUMENT_CLIP_VALUE_SP);
#endif
#if __GRF_POSTVENTION__
					static const __m512 float_min = _mm512_set1_ps(FLT_MIN);
#endif
					for (; j < size.width; j += 64)//64 pixel unit
					{
						...
						for (k = 0; k < maxk; k++, ofs++)
						{
							...
							__m512 aw = _mm512_mul_ps(difft, mcw);
#if __GRF_PREVENTION__
							aw = _mm512_max_ps(aw, exp_min_arg);
#endif
							__m512 _w =
#if __USE_INTEL_EXP__
								_mm512_exp_ps(aw);
...
*************************************************/
class GaussianRangeFilter_LUT_Setx3_64f_InvokerAVX512 : public cv::ParallelLoopBody
{
public:
	GaussianRangeFilter_LUT_Setx3_64f_InvokerAVX512(Mat& _dest, const Mat& _temp, const int _radiusH, const int _radiusV, const int _maxk, const int* _space_ofs, const double *_range_weight, const double offset = 1) :
	...
	void operator() (const Range& range) const override
	{
		...
		else
		{
			...
			for (i = range.start; i != range.end; i++, sptrr += sstep, sptrg += sstep, sptrb += sstep, dptr += dstep)
			{
				...
				{
#if __GRF_PREVENTION__
					const __m512d moffset = _mm512_set1_pd(offset);
#endif
#if __GRF_POSTVENTION__
					static const __m512d double_min = _mm512_set1_pd(DBL_MIN);
#endif
...
class GaussianRangeFilter_LUT_Setx3_32f_InvokerAVX512 : public cv::ParallelLoopBody
{
public:
	GaussianRangeFilter_LUT_Setx3_32f_InvokerAVX512(Mat& _dest, const Mat& _temp, const int _radiusH, const int _radiusV, const int _maxk, const int* _space_ofs, const float *_range_weight, const float offset = 1) :
		temp(&_temp), dest(&_dest), radiusH(_radiusH), radiusV(_radiusV), maxk(_maxk), space_ofs(_space_ofs), range_weight(_range_weight), offset(offset)
	{
	}
	...
	void operator() (const Range& range) const override
	{
		...
		else
		{
			...
			for (i = range.start; i != range.end; i++, sptrr += sstep, sptrg += sstep, sptrb += sstep, dptr += dstep)
			{
				...
				{
#if __GRF_PREVENTION__
					const __m512 moffset = _mm512_set1_ps(offset);
#endif
#if __GRF_POSTVENTION__
					static const __m512 float_min = _mm512_set1_ps(FLT_MIN);
#endif
...
class GaussianRangeFilter_LUT_Setx3_8u_InvokerAVX512 : public cv::ParallelLoopBody
{
public:
	GaussianRangeFilter_LUT_Setx3_8u_InvokerAVX512(Mat& _dest, const Mat& _temp, const int _radiusH, const int _radiusV, const int _maxk, const int* _space_ofs, const float *_range_weight, const float offset = 1) :
		temp(&_temp), dest(&_dest), radiusH(_radiusH), radiusV(_radiusV), maxk(_maxk), space_ofs(_space_ofs), range_weight(_range_weight), offset(offset)
	{
	}
		...
		else
		{
			...
			for (i = range.start; i != range.end; i++, sptrr += sstep, sptrg += sstep, sptrb += sstep, dptr += dstep)
			{
				...
				for (; j < size.width; j++)
				{
					...
					{
						const float r = sptrrj[space_ofs[k]], g = sptrgj[space_ofs[k]], b = sptrbj[space_ofs[k]];
						const float w = range_weight[(int)abs(b - b0)] * range_weight[(int)abs(g - g0)] * range_weight[(int)abs(r - r0)];
						sum_b += b * w;
						sum_g += g * w;
						sum_r += r * w;
						wsum += w;
...
**************************************************/
class GaussianRangeFilter_LUT_Gatherx3_64f_InvokerAVX512 : public cv::ParallelLoopBody
{
public:
	GaussianRangeFilter_LUT_Gatherx3_64f_InvokerAVX512(Mat& _dest, const Mat& _temp, const int _radiusH, const int _radiusV, const int _maxk, const int* _space_ofs, const double* _range_weight, const double offset = 1) :
		temp(&_temp), dest(&_dest), radiusH(_radiusH), radiusV(_radiusV), maxk(_maxk), space_ofs(_space_ofs), range_weight(_range_weight), offset(offset)
	{
	}
				...
				if (haveAVX512)
				{
					for (; j < size.width; j += 8)//8 pixel unit
					{
						...
						for (k = 0; k < maxk; k++, ofs++)
						{
							...
							__m512d _w = _mm512_i64gather_pd(diff, range_weight, 8);
#if __USE_FMA_INSTRUCTION__
							tval = _mm512_fmadd_pd(sref, _w, tval);
#else
							sref = _mm512_mul_pd(sref, _w);
		...
		else
		{
			...
			for (i = range.start; i != range.end; i++, sptrr += sstep, sptrg += sstep, sptrb += sstep, dptr += dstep)
			{
				...
				if (haveAVX512)
				{
					...
					for (; j < size.width; j += 8)//8 pixel unit
					{
						...
						for (k = 0; k < maxk; k++, ofs++)
						{
							...
							__m512d _w = _mm512_mul_pd(
								moffset,
								_mm512_i64gather_pd(bdiff, range_weight, 8)
							);
#else
							__m512d _w = _mm512_i64gather_pd(bdiff, range_weight, 8);
#endif
				...
				for (; j < size.width; j++)
				{
					...
					{
						const double r = sptrrj[space_ofs[k]], g = sptrgj[space_ofs[k]], b = sptrbj[space_ofs[k]];
						const double w = range_weight[(int)abs(b - b0)] * range_weight[(int)abs(g - g0)] * range_weight[(int)abs(r - r0)];
						sum_b += b * w;
						sum_g += g * w;
						sum_r += r * w;
						wsum += w;
...
class GaussianRangeFilter_LUT_Gatherx3_32f_InvokerAVX512 : public cv::ParallelLoopBody
{
public:
	GaussianRangeFilter_LUT_Gatherx3_32f_InvokerAVX512(Mat& _dest, const Mat& _temp, const int _radiusH, const int _radiusV, const int _maxk, const int* _space_ofs, const float *_range_weight, const float offset = 1) :
		temp(&_temp), dest(&_dest), radiusH(_radiusH), radiusV(_radiusV), maxk(_maxk), space_ofs(_space_ofs), range_weight(_range_weight), offset(offset)
	{
	}
				...
				if (haveAVX512)
				{
					for (; j < size.width; j += 16)//16 pixel unit
					{
						...
						for (k = 0; k < maxk; k++, ofs++)
						{
							...
							__m512 _w = _mm512_i32gather_ps(diff, range_weight, 4);
#if __USE_FMA_INSTRUCTION__
							tval = _mm512_fmadd_ps(sref, _w, tval);
#else
							sref = _mm512_mul_ps(sref, _w);
				...
				if (haveAVX512)
				{
					...
					for (; j < size.width; j += 16)//16 pixel unit
					{
						...
						for (k = 0; k < maxk; k++, ofs++)
						{
							...
							__m512 _w = _mm512_mul_ps(
								moffset,
								_mm512_i32gather_ps(bdiff, range_weight, 4)
							);
#else
							__m512 _w = _mm512_i32gather_ps(bdiff, range_weight, 4);
#endif
...
class GaussianRangeFilter_LUT_Gatherx3_8u_InvokerAVX512 : public cv::ParallelLoopBody
{
public:
	GaussianRangeFilter_LUT_Gatherx3_8u_InvokerAVX512(Mat& _dest, const Mat& _temp, const int _radiusH, const int _radiusV, const int _maxk, const int* _space_ofs, const float *_range_weight, const float offset = 1) :
		temp(&_temp), dest(&_dest), radiusH(_radiusH), radiusV(_radiusV), maxk(_maxk), space_ofs(_space_ofs), range_weight(_range_weight), offset(offset)
	{
	}
				...
				if (haveAVX512)
				{
					...
					for (; j < size.width; j += 64)//64 pixel unit
					{
						...
						for (k = 0; k < maxk; k++, ofs++)
						{
							...
							_w = _mm512_i32gather_ps(diff, range_weight, 4);
							sref = _mm512_cvtepi32_ps(_mm512_unpackhi_epi16(sref_32elem, zero));
#if __USE_FMA_INSTRUCTION__
							tval2 = _mm512_fmadd_ps(sref, _w, tval2);
#else
...
#else
							_w = _mm512_i32gather_ps(bdiff, range_weight, 4);
#endif
							_w = _mm512_mul_ps(_w, _mm512_i32gather_ps(gdiff, range_weight, 4));
#if __GRF_POSTVENTION__
							_w = _mm512_max_ps(_w, float_min);
...
**************************************************/
class GaussianRangeFilter_QuantizationRangeLUT_Setx1_64f_InvokerAVX512 : public cv::ParallelLoopBody
{
public:
	GaussianRangeFilter_QuantizationRangeLUT_Setx1_64f_InvokerAVX512(Mat& _dest, const Mat& _temp, const int _radiusH, const int _radiusV, const int _maxk, const int* _space_ofs, const double* _range_weight) :
...
class GaussianRangeFilter_QuantizationRangeLUT_Setx1_32f_InvokerAVX512 : public cv::ParallelLoopBody
{
public:
	GaussianRangeFilter_QuantizationRangeLUT_Setx1_32f_InvokerAVX512(Mat& _dest, const Mat& _temp, const int _radiusH, const int _radiusV, const int _maxk, const int* _space_ofs, const float *_range_weight) :
		temp(&_temp), dest(&_dest), radiusH(_radiusH), radiusV(_radiusV), maxk(_maxk), space_ofs(_space_ofs), range_weight(_range_weight)
	{
	}
...
class GaussianRangeFilter_QuantizationRangeLUT_Setx1_8u_InvokerAVX512 : public cv::ParallelLoopBody
{
public:
	GaussianRangeFilter_QuantizationRangeLUT_Setx1_8u_InvokerAVX512(Mat& _dest, const Mat& _temp, const int _radiusH, const int _radiusV, const int _maxk, const int* _space_ofs, const  float *_range_weight) :
		temp(&_temp), dest(&_dest), radiusH(_radiusH), radiusV(_radiusV), maxk(_maxk), space_ofs(_space_ofs), range_weight(_range_weight)
	{
	}
...
*************************************************/
class GaussianRangeFilter_QuantizationRangeLUT_Gatherx1_64f_InvokerAVX512 : public cv::ParallelLoopBody
{
public:
	GaussianRangeFilter_QuantizationRangeLUT_Gatherx1_64f_InvokerAVX512(Mat& _dest, const Mat& _temp, const int _radiusH, const int _radiusV, const int _maxk, const int* _space_ofs, const double *_range_weight) :
		temp(&_temp), dest(&_dest), radiusH(_radiusH), radiusV(_radiusV), maxk(_maxk), space_ofs(_space_ofs), range_weight(_range_weight)
	{
	}
...
class GaussianRangeFilter_QuantizationRangeLUT_Gatherx1_32f_InvokerAVX512 : public cv::ParallelLoopBody
{
public:
	GaussianRangeFilter_QuantizationRangeLUT_Gatherx1_32f_InvokerAVX512(Mat& _dest, const Mat& _temp, const int _radiusH, const int _radiusV, const int _maxk, const int* _space_ofs, const float *_range_weight) :
		temp(&_temp), dest(&_dest), radiusH(_radiusH), radiusV(_radiusV), maxk(_maxk), space_ofs(_space_ofs), range_weight(_range_weight)
	{
	}
...
class GaussianRangeFilter_QuantizationRangeLUT_Gatherx1_8u_InvokerAVX512 : public cv::ParallelLoopBody
{
public:
	GaussianRangeFilter_QuantizationRangeLUT_Gatherx1_8u_InvokerAVX512(Mat& _dest, const Mat& _temp, const int _radiusH, const int _radiusV, const int _maxk, const int* _space_ofs, const float *_range_weight) :
		temp(&_temp), dest(&_dest), radiusH(_radiusH), radiusV(_radiusV), maxk(_maxk), space_ofs(_space_ofs), range_weight(_range_weight)
	{
	}
...
{
	//////////////////////////////////////////////////////////////////
	void gaussianRangeFilter_AVX512_64f(const Mat& src, Mat& dst, const Size kernelSize, double sigma_range, const int borderType, const bool isRectangle, const WEIGHT_MODE weightingMethod)
	{
		...
		case WEIGHT_VECTOR_EXP:
		{
			...
			const GaussianRangeFilter_EXP_64f_InvokerAVX512 body(dest, temp, radiusH, radiusV, maxk, space_ofs, sigma_range);
		...
		case WEIGHT_RANGE_LUT_SET:
		{
			...
			const GaussianRangeFilter_LUT_Setx3_64f_InvokerAVX512 body(dest, temp, radiusH, radiusV, maxk, space_ofs, range_weight, bias);
			parallel_for_(Range(0, size.height), body);
			break;
		}
		case WEIGHT_RANGE_LUT_GATHER:
		{
			...
			// initialize color-related gaussianRange filter coefficients
#if __GRF_PREVENTION__
			const double max_digits = floor(log2(DBL_MAX / (255.f*kernelSize.area())) - log2(DBL_MIN));
			const double bias_digits = floor((max_digits - cn) / cn);
			const double bias = pow(2, log2(DBL_MAX / (255.f*kernelSize.area())));
			const double exp_clip_val = log(pow(2, -bias_digits)) + DBL_EPSILON;
#else
			const double bias = 1;
#endif
			...
			const GaussianRangeFilter_LUT_Gatherx3_64f_InvokerAVX512 body(dest, temp, radiusH, radiusV, maxk, space_ofs, range_weight, bias);
			parallel_for_(Range(0, size.height), body);
			break;
		}
		...
		{
			// initialize space-related gaussianRange filter coefficients
			const int value_range = 442;	//floor(sqrt(255^2*3))+1
			vector<double> _range_weight(value_range);
			double* range_weight = &_range_weight[0];
			...
			const GaussianRangeFilter_QuantizationRangeLUT_Setx1_64f_InvokerAVX512 body(dest, temp, radiusH, radiusV, maxk, space_ofs, range_weight);
			parallel_for_(Range(0, size.height), body);
			break;
		}
		case WEIGHT_RANGE_QUANTIZATION_LUT_GATHER:
		{
			...
			const GaussianRangeFilter_QuantizationRangeLUT_Gatherx1_64f_InvokerAVX512 body(dest, temp, radiusH, radiusV, maxk, space_ofs, range_weight);
			parallel_for_(Range(0, size.height), body);
			break;
		}
		default:
	...
	//////////////////////////////////////////////////////////////////
	void gaussianRangeFilter_AVX512_32f(const Mat& src, Mat& dst, const Size kernelSize, float sigma_range, const int borderType, const bool isRectangle, const WEIGHT_MODE weightingMethod)
	{
		if (kernelSize.width == 0 || kernelSize.height == 0) { src.copyTo(dst); return; }
		const int cn = src.channels();
		const Size size = src.size();
		...
		{
			// initialize space-related gaussianRange filter coefficients
			const GaussianRangeFilter_EXP_32f_InvokerAVX512 body(dest, temp, radiusH, radiusV, maxk, space_ofs, sigma_range);
			parallel_for_(Range(0, size.height), body);
			break;
		}
		case WEIGHT_RANGE_LUT_SET:
		{
			...
			const GaussianRangeFilter_LUT_Setx3_32f_InvokerAVX512 body(dest, temp, radiusH, radiusV, maxk, space_ofs, range_weight, bias);
			parallel_for_(Range(0, size.height), body);
			break;
		}
		case WEIGHT_RANGE_LUT_GATHER:
		{
			...
			const GaussianRangeFilter_LUT_Gatherx3_32f_InvokerAVX512 body(dest, temp, radiusH, radiusV, maxk, space_ofs, range_weight, bias);
			parallel_for_(Range(0, size.height), body);
			break;
		}
		case WEIGHT_RANGE_QUANTIZATION_LUT_SET:
		{
			...
			const GaussianRangeFilter_QuantizationRangeLUT_Setx1_32f_InvokerAVX512 body(dest, temp, radiusH, radiusV, maxk, space_ofs, range_weight);
			parallel_for_(Range(0, size.height), body);
			break;
		}
		case WEIGHT_RANGE_QUANTIZATION_LUT_GATHER:
		{
			...
			const GaussianRangeFilter_QuantizationRangeLUT_Gatherx1_32f_InvokerAVX512 body(dest, temp, radiusH, radiusV, maxk, space_ofs, range_weight);
			parallel_for_(Range(0, size.height), body);
			break;
		}
		default:
	...
	//////////////////////////////////////////////////////////////////
	void gaussianRangeFilter_AVX512_8u(const Mat& src, Mat& dst, const Size kernelSize, float sigma_range, const int borderType, const bool isRectangle, const WEIGHT_MODE weightingMethod)
	{
		if (kernelSize.width == 0 || kernelSize.height == 0) { src.copyTo(dst); return; }
		const int cn = src.channels();
		const Size size = src.size();
		...
		{
			// initialize space-related gaussianRange filter coefficients
			const GaussianRangeFilter_EXP_8u_InvokerAVX512 body(dest, temp, radiusH, radiusV, maxk, space_ofs, sigma_range);
			parallel_for_(Range(0, size.height), body);
			break;
		}
		case WEIGHT_RANGE_LUT_SET:
		{
			...
			const GaussianRangeFilter_LUT_Setx3_8u_InvokerAVX512 body(dest, temp, radiusH, radiusV, maxk, space_ofs, range_weight, bias);
			parallel_for_(Range(0, size.height), body);
			break;
		}
		case WEIGHT_RANGE_LUT_GATHER:
		{
			...
			const GaussianRangeFilter_LUT_Gatherx3_8u_InvokerAVX512 body(dest, temp, radiusH, radiusV, maxk, space_ofs, range_weight, bias);
			parallel_for_(Range(0, size.height), body);
			break;
		}
		case WEIGHT_RANGE_QUANTIZATION_LUT_SET:
		{
			...
			const GaussianRangeFilter_QuantizationRangeLUT_Setx1_8u_InvokerAVX512 body(dest, temp, radiusH, radiusV, maxk, space_ofs, range_weight);
			parallel_for_(Range(0, size.height), body);
			break;
		}
		case WEIGHT_RANGE_QUANTIZATION_LUT_GATHER:
		{
			...
			const GaussianRangeFilter_QuantizationRangeLUT_Gatherx1_8u_InvokerAVX512 body(dest, temp, radiusH, radiusV, maxk, space_ofs, range_weight);
			parallel_for_(Range(0, size.height), body);
			break;
		}
		default:
	 ...
	 */
	void gaussianRangeFilter_AVX512(cv::InputArray src_, cv::OutputArray dest, const cv::Size kernelSize, const double sigma_range, const int kernel_type, const int borderType, const WEIGHT_MODE weightingMethod)
	{
		if (dest.empty() || dest.type() != src_.type() || dest.size() != src_.size()) dest.create(src_.size(), src_.type());
		Mat src = src_.getMat();
		Mat dst = dest.getMat();

		if (kernel_type == FILTER_RECTANGLE)
		{
			if (src.depth() == CV_8U)
			{
				gaussianRangeFilter_AVX512_8u(src, dst, kernelSize, sigma_range, borderType, true, weightingMethod);
			}
			else if (src.depth() == CV_32F)
			{
				gaussianRangeFilter_AVX512_32f(src, dst, kernelSize, sigma_range, borderType, true, weightingMethod);
			}
			else if (src.depth() == CV_64F)
			{
				gaussianRangeFilter_AVX512_64f(src, dst, kernelSize, sigma_range, borderType, true, weightingMethod);
			}
		}
		else if (kernel_type == FILTER_CIRCLE)
		{
			if (src.depth() == CV_8U)
			{
				gaussianRangeFilter_AVX512_8u(src, dst, kernelSize, sigma_range, borderType, false, weightingMethod);
			}
			else if (src.depth() == CV_32F)
			{
				gaussianRangeFilter_AVX512_32f(src, dst, kernelSize, sigma_range, borderType, false, weightingMethod);
			}
			else if (src.depth() == CV_64F)
			{
				gaussianRangeFilter_AVX512_64f(src, dst, kernelSize, sigma_range, borderType, false, weightingMethod);
			}
		}
	}
}
