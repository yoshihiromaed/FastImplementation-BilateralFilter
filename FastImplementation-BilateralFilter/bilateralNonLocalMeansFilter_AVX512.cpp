...
/*************************************************
	using exp function
*************************************************/
class BilateralNonlocalMeansFilterInvorker_EXP_64f_AVX512 : public cv::ParallelLoopBody
{
private:
	const Mat * src;
	Mat* dest;
		...
		if (dest->channels() == 3)
		{
			...
			for (int j = range.start; j < range.end; j++)
			{
				...
				if (haveAVX512)
				{
					...
					const __m512d gsc = _mm512_set1_pd(gauss_space_coeff);
					...
					for (; i < dest->cols; i += 8)
					{
						...
						for (int l = 0; l < search_maxk; l++, spw++)
						{
							...
							const __m512d _sw = _mm512_mul_pd(_mm512_set1_pd(*spw), gsc);
							const __m512d _cw = _mm512_mul_pd(me, gcc);
							__m512d aw = _mm512_add_pd(_sw, _cw);
#if __BNLMF_PREVENTION__
...
class BilateralNonlocalMeansFilterInvorker_EXP_32f_AVX512 : public cv::ParallelLoopBody
{
private:
	const Mat * src;
	Mat* dest;
		...
		if (dest->channels() == 3)
		{
			...
			for (int j = range.start; j < range.end; j++)
			{
				...
				if (haveAVX512)
				{
					...
					const __m512 gsc = _mm512_set1_ps(gauss_space_coeff);
#if __BNLMF_PREVENTION__
					...
					for (; i < dest->cols; i += 16)
					{
						...
						for (int l = 0; l < search_maxk; l++, spw++)
						{
							...
							__m512 _sw = _mm512_mul_ps(_mm512_set1_ps(*spw), gsc);
							__m512 _cw = _mm512_mul_ps(me, gcc);
							__m512 aw = _mm512_add_ps(_sw, _cw);
#if __BNLMF_PREVENTION__
				...
				if (haveAVX512)
				{
					...
					for (; i < dest->cols; i += 16)
					{
						...
						for (int l = 0; l < search_maxk; l++, spw++)
						{
							...
							const __m512 _sw = _mm512_mul_ps(_mm512_set1_ps(*spw), gsc);
							const __m512 _cw = _mm512_mul_ps(me, gcc);
							__m512 aw = _mm512_add_ps(_sw, _cw);
#if __BNLMF_PREVENTION__
							aw = _mm512_max_ps(aw, exp_arg_min);
#endif
...
class BilateralNonlocalMeansFilterInvorker_EXP_8u_AVX512 : public cv::ParallelLoopBody
{
private:
	const Mat * src;
	Mat* dest;
...
public:
	BilateralNonlocalMeansFilterInvorker_EXP_8u_AVX512(const Mat& src_, Mat& dest_, const int templateWindowSizeX_, const int templateWindowSizeY_, const int searchWindowSizeX_, const int searchWindowSizeY_, const float h, const float sigma_space, const float* space_weight, const int template_maxk, const int* template_ofs, const int search_maxk, const int* search_ofs)
		: src(&src_), dest(&dest_), template_ofs(template_ofs), template_maxk(template_maxk), search_ofs(search_ofs), search_maxk(search_maxk), templateWindowSizeX(templateWindowSizeX_), templateWindowSizeY(templateWindowSizeY_), searchWindowSizeX(searchWindowSizeX_), searchWindowSizeY(searchWindowSizeY_), h(h), sigma_space(sigma_space), space_weight(space_weight)
	{
	}
				...
				if (haveAVX512)
				{
					...
					for (; i < dest->cols; i += 64)
					{
						...
						for (int l = 0; l < search_maxk; l++, spw++)
						{
							...
							const __m512 _sw = _mm512_mul_ps(_mm512_set1_ps(*spw), gsc);
							//low low
							__m512 _cw = _mm512_mul_ps(me0, gcc);
							__m512 aw = _mm512_add_ps(_sw, _cw);
#if __BNLMF_PREVENTION__
							aw = _mm512_max_ps(aw, exp_arg_min);
#endif
							...
							//low high
							_cw = _mm512_mul_ps(me1, gcc);
							aw = _mm512_add_ps(_sw, _cw);
#if __BNLMF_PREVENTION__
							aw = _mm512_max_ps(aw, exp_arg_min);
#endif
							mw =
...
*************************************************/
class BilateralNonlocalMeansFilterInvorker_EXP_WITH_SPACE_LUT_64f_AVX512 : public cv::ParallelLoopBody
{
private:
	const Mat * src;
	Mat* dest;
...
public:
	BilateralNonlocalMeansFilterInvorker_EXP_WITH_SPACE_LUT_64f_AVX512(const Mat& src_, Mat& dest_, const int templateWindowSizeX_, const int templateWindowSizeY_, const int searchWindowSizeX_, const int searchWindowSizeY_, const double h, const double* space_weight, const int template_maxk, const int* template_ofs, const int search_maxk, const int* search_ofs, const double exp_clip_val = -1000000)
...
class BilateralNonlocalMeansFilterInvorker_EXP_WITH_SPACE_LUT_32f_AVX512 : public cv::ParallelLoopBody
{
private:
	const Mat * src;
	Mat* dest;
...
class BilateralNonlocalMeansFilterInvorker_EXP_WITH_SPACE_LUT_8u_AVX512 : public cv::ParallelLoopBody
{
private:
	const Mat * src;
	Mat* dest;
...
public:
	BilateralNonlocalMeansFilterInvorker_EXP_WITH_SPACE_LUT_8u_AVX512(const Mat& src_, Mat& dest_, const int templateWindowSizeX_, const int templateWindowSizeY_, const int searchWindowSizeX_, const int searchWindowSizeY_, const float h, const float* space_weight, const int template_maxk, const int* template_ofs, const int search_maxk, const int* search_ofs, const float exp_clip_val = -200)
		: src(&src_), dest(&dest_), template_ofs(template_ofs), template_maxk(template_maxk), search_ofs(search_ofs), search_maxk(search_maxk), templateWindowSizeX(templateWindowSizeX_), templateWindowSizeY(templateWindowSizeY_), searchWindowSizeX(searchWindowSizeX_), searchWindowSizeY(searchWindowSizeY_), h(h), space_weight(space_weight), exp_clip_val(exp_clip_val)
	{
	}
...
*************************************************/
class BilateralNonlocalMeansFilterInvorker_LUT_Setx3_64f_AVX512 : public cv::ParallelLoopBody
{
private:
	const Mat * src;
	Mat* dest;
...
class BilateralNonlocalMeansFilterInvorker_LUT_Setx3_32f_AVX512 : public cv::ParallelLoopBody
{
private:
	const Mat * src;
	Mat* dest;
...
class BilateralNonlocalMeansFilterInvorker_LUT_Setx3_8u_AVX512 : public cv::ParallelLoopBody
{
private:
	const Mat * src;
	Mat* dest;
...
public:
	BilateralNonlocalMeansFilterInvorker_LUT_Setx3_8u_AVX512(const Mat& src_, Mat& dest_, const int templateWindowSizeX_, const int templateWindowSizeY_, const int searchWindowSizeX_, const int searchWindowSizeY_, const float* weight, const float* space_weight, const int template_maxk, const int* template_ofs, const int search_maxk, const int* search_ofs)
		: src(&src_), dest(&dest_), template_ofs(template_ofs), template_maxk(template_maxk), search_ofs(search_ofs), search_maxk(search_maxk), templateWindowSizeX(templateWindowSizeX_), templateWindowSizeY(templateWindowSizeY_), searchWindowSizeX(searchWindowSizeX_), searchWindowSizeY(searchWindowSizeY_), weight(weight), space_weight(space_weight)
	{
	}
				...
				if (haveAVX512)
				{
					...
					for (; i < dest->cols; i += 64)
					{
						...
						for (int l = 0; l < search_maxk; l++, spw++)
						{
							...
							const uchar* s = (sptr + s_ofs[l]);
							count = 0;
							__m512 mw0 = _mm512_set1_ps(*spw);
							__m512 mw1 = _mm512_set1_ps(*spw);
							__m512 mw2 = _mm512_set1_ps(*spw);
							__m512 mw3 = _mm512_set1_ps(*spw);
							for (int n = 0; n < template_maxk; n++)
							{
								// computing color L2 norm
								//b
...
*************************************************/
class BilateralNonlocalMeansFilterInvorker_QuantizationRangeLUT_SetxN_64f_AVX512 : public cv::ParallelLoopBody
{
private:
	const Mat * src;
	Mat* dest;
...
public:
	BilateralNonlocalMeansFilterInvorker_QuantizationRangeLUT_SetxN_64f_AVX512(const Mat& src_, Mat& dest_, const int templateWindowSizeX_, const int templateWindowSizeY_, const int searchWindowSizeX_, const int searchWindowSizeY_, const double* weight, const double* space_weight, const int template_maxk, const int* template_ofs, const int search_maxk, const int* search_ofs)
		: src(&src_), dest(&dest_), template_ofs(template_ofs), template_maxk(template_maxk), search_maxk(search_maxk), search_ofs(search_ofs), templateWindowSizeX(templateWindowSizeX_), templateWindowSizeY(templateWindowSizeY_), searchWindowSizeX(searchWindowSizeX_), searchWindowSizeY(searchWindowSizeY_), weight(weight), space_weight(space_weight)
	{
	}
...
class BilateralNonlocalMeansFilterInvorker_QuantizationRangeLUT_SetxN_32f_AVX512 : public cv::ParallelLoopBody
{
private:
	const Mat * src;
	Mat* dest;
...
public:
	BilateralNonlocalMeansFilterInvorker_QuantizationRangeLUT_SetxN_32f_AVX512(const Mat& src_, Mat& dest_, const int templateWindowSizeX_, const int templateWindowSizeY_, const int searchWindowSizeX_, const int searchWindowSizeY_, const float* weight, const float* space_weight, const int template_maxk, const int* template_ofs, const int search_maxk, const int* search_ofs)
		: src(&src_), dest(&dest_), template_ofs(template_ofs), template_maxk(template_maxk), search_ofs(search_ofs), search_maxk(search_maxk), templateWindowSizeX(templateWindowSizeX_), templateWindowSizeY(templateWindowSizeY_), searchWindowSizeX(searchWindowSizeX_), searchWindowSizeY(searchWindowSizeY_), weight(weight), space_weight(space_weight)
	{
	}
...
class BilateralNonlocalMeansFilterInvorker_QuantizationRangeLUT_SetxN_8u_AVX512 : public cv::ParallelLoopBody
{
private:
	const Mat * src;
	Mat* dest;
...
public:
	BilateralNonlocalMeansFilterInvorker_QuantizationRangeLUT_SetxN_8u_AVX512(const Mat& src_, Mat& dest_, const int templateWindowSizeX_, const int templateWindowSizeY_, const int searchWindowSizeX_, const int searchWindowSizeY_, const float* weight, const float* space_weight, const int template_maxk, const int* template_ofs, const int search_maxk, const int* search_ofs)
		: src(&src_), dest(&dest_), template_ofs(template_ofs), template_maxk(template_maxk), search_ofs(search_ofs), search_maxk(search_maxk), templateWindowSizeX(templateWindowSizeX_), templateWindowSizeY(templateWindowSizeY_), searchWindowSizeX(searchWindowSizeX_), searchWindowSizeY(searchWindowSizeY_), weight(weight), space_weight(space_weight)
	{
	}
...
*************************************************/
class BilateralNonlocalMeansFilterInvorker_QuantizationRangeLUT_Setx1_64f_AVX512 : public cv::ParallelLoopBody
{
private:
	const Mat * src;
	Mat* dest;
...
public:
	BilateralNonlocalMeansFilterInvorker_QuantizationRangeLUT_Setx1_64f_AVX512(const Mat& src_, Mat& dest_, const int templateWindowSizeX_, const int templateWindowSizeY_, const int searchWindowSizeX_, const int searchWindowSizeY_, const double* weight, const double* space_weight, const int template_maxk, const int* template_ofs, const int search_maxk, const int* search_ofs)
		: src(&src_), dest(&dest_), template_ofs(template_ofs), template_maxk(template_maxk), search_maxk(search_maxk), search_ofs(search_ofs), templateWindowSizeX(templateWindowSizeX_), templateWindowSizeY(templateWindowSizeY_), searchWindowSizeX(searchWindowSizeX_), searchWindowSizeY(searchWindowSizeY_), weight(weight), space_weight(space_weight)
	{
	}
				...
				if (haveAVX512)
				{
					...
					for (; i < dest->cols; i += 8)
					{
						...
						for (int l = 0; l < search_maxk; l++, spw++)
						{
							...
							_mm512_store_si512((__m512i*)buf, _mm512_cvtpd_epi64(_mm512_maskz_rcp14_pd((__mmask8)_MM_K0_REG, _mm512_maskz_rsqrt14_pd((__mmask8)_MM_K0_REG, me))));
							const __m512d _sw = _mm512_set1_pd(*spw);
							__m512d mw = _mm512_mul_pd(_sw, _mm512_set_pd(w[buf[7]], w[buf[6]], w[buf[5]], w[buf[4]], w[buf[3]], w[buf[2]], w[buf[1]], w[buf[0]]));
#if __BNLMF_POSTVENTION__
							mw = _mm512_max_pd(mw, double_min);
#endif
#if __USE_FMA_INSTRUCTION__
...
class BilateralNonlocalMeansFilterInvorker_QuantizationRangeLUT_Setx1_32f_AVX512 : public cv::ParallelLoopBody
{
private:
	const Mat * src;
	Mat* dest;
...
public:
	BilateralNonlocalMeansFilterInvorker_QuantizationRangeLUT_Setx1_32f_AVX512(const Mat& src_, Mat& dest_, const int templateWindowSizeX_, const int templateWindowSizeY_, const int searchWindowSizeX_, const int searchWindowSizeY_, const float* weight, const float* space_weight, const int template_maxk, const int* template_ofs, const int search_maxk, const int* search_ofs)
		: src(&src_), dest(&dest_), template_ofs(template_ofs), template_maxk(template_maxk), search_ofs(search_ofs), search_maxk(search_maxk), templateWindowSizeX(templateWindowSizeX_), templateWindowSizeY(templateWindowSizeY_), searchWindowSizeX(searchWindowSizeX_), searchWindowSizeY(searchWindowSizeY_), weight(weight), space_weight(space_weight)
	{
	}
				...
				if (haveAVX512)
				{
					...
					for (; i < dest->cols; i += 16)
					{
						...
						for (int l = 0; l < search_maxk; l++, spw++)
						{
							...
							_mm512_store_si512((__m512i*)buf, _mm512_cvtps_epi32(_mm512_rcp14_ps(_mm512_rsqrt14_ps(me))));
							const __m512 _sw = _mm512_set1_ps(*spw);
							__m512 mw = _mm512_mul_ps(_sw, _mm512_set_ps(
								w[buf[15]], w[buf[14]], w[buf[13]], w[buf[12]], w[buf[11]], w[buf[10]], w[buf[9]], w[buf[8]],
								w[buf[7]], w[buf[6]], w[buf[5]], w[buf[4]], w[buf[3]], w[buf[2]], w[buf[1]], w[buf[0]]
							));
#if __BNLMF_POSTVENTION__
...
class BilateralNonlocalMeansFilterInvorker_QuantizationRangeLUT_Setx1_8u_AVX512 : public cv::ParallelLoopBody
{
private:
	const Mat * src;
	Mat* dest;
...
public:
	BilateralNonlocalMeansFilterInvorker_QuantizationRangeLUT_Setx1_8u_AVX512(const Mat& src_, Mat& dest_, const int templateWindowSizeX_, const int templateWindowSizeY_, const int searchWindowSizeX_, const int searchWindowSizeY_, const float* weight, const float* space_weight, const int template_maxk, const int* template_ofs, const int search_maxk, const int* search_ofs)
		: src(&src_), dest(&dest_), template_ofs(template_ofs), template_maxk(template_maxk), search_ofs(search_ofs), search_maxk(search_maxk), templateWindowSizeX(templateWindowSizeX_), templateWindowSizeY(templateWindowSizeY_), searchWindowSizeX(searchWindowSizeX_), searchWindowSizeY(searchWindowSizeY_), weight(weight), space_weight(space_weight)
	{
	}
...
*************************************************/
class BilateralNonlocalMeansFilterInvorker_QuantizationLUT_Setx1_64f_AVX512 : public cv::ParallelLoopBody
{
private:
	const Mat * src;
	Mat* dest;
...
class BilateralNonlocalMeansFilterInvorker_QuantizationLUT_Setx1_32f_AVX512 : public cv::ParallelLoopBody
{
private:
	const Mat * src;
	Mat* dest;
...
class BilateralNonlocalMeansFilterInvorker_QuantizationLUT_Setx1_8u_AVX512 : public cv::ParallelLoopBody
{
private:
	const Mat * src;
	Mat* dest;
...
public:
	BilateralNonlocalMeansFilterInvorker_QuantizationLUT_Setx1_8u_AVX512(const Mat& src_, Mat& dest_, const int templateWindowSizeX_, const int templateWindowSizeY_, const int searchWindowSizeX_, const int searchWindowSizeY_, const float h, const float sigma_space, const float* weight, const float* space_weight, const int template_maxk, const int* template_ofs, const int search_maxk, const int* search_ofs)
...
*************************************************/
class BilateralNonlocalMeansFilterInvorker_LUT_Gatherx3_64f_AVX512 : public cv::ParallelLoopBody
{
private:
	const Mat * src;
	Mat* dest;
...
public:
	BilateralNonlocalMeansFilterInvorker_LUT_Gatherx3_64f_AVX512(const Mat& src_, Mat& dest_, const int templateWindowSizeX_, const int templateWindowSizeY_, const int searchWindowSizeX_, const int searchWindowSizeY_, const double* weight, const double* space_weight, const int template_maxk, const int* template_ofs, const int search_maxk, const int* search_ofs)
		: src(&src_), dest(&dest_), template_ofs(template_ofs), template_maxk(template_maxk), search_maxk(search_maxk), search_ofs(search_ofs), templateWindowSizeX(templateWindowSizeX_), templateWindowSizeY(templateWindowSizeY_), searchWindowSizeX(searchWindowSizeX_), searchWindowSizeY(searchWindowSizeY_), weight(weight), space_weight(space_weight)
	{
	}
...
class BilateralNonlocalMeansFilterInvorker_LUT_Gatherx3_32f_AVX512 : public cv::ParallelLoopBody
{
private:
	const Mat * src;
	Mat* dest;
...
public:
	BilateralNonlocalMeansFilterInvorker_LUT_Gatherx3_32f_AVX512(const Mat& src_, Mat& dest_, const int templateWindowSizeX_, const int templateWindowSizeY_, const int searchWindowSizeX_, const int searchWindowSizeY_, const float* weight, const float* space_weight, const int template_maxk, const int* template_ofs, const int search_maxk, const int* search_ofs)
		: src(&src_), dest(&dest_), template_ofs(template_ofs), template_maxk(template_maxk), search_ofs(search_ofs), search_maxk(search_maxk), templateWindowSizeX(templateWindowSizeX_), templateWindowSizeY(templateWindowSizeY_), searchWindowSizeX(searchWindowSizeX_), searchWindowSizeY(searchWindowSizeY_), weight(weight), space_weight(space_weight)
	{
	}
...
class BilateralNonlocalMeansFilterInvorker_LUT_Gatherx3_8u_AVX512 : public cv::ParallelLoopBody
{
private:
	const Mat * src;
	Mat* dest;
...
public:
	BilateralNonlocalMeansFilterInvorker_LUT_Gatherx3_8u_AVX512(const Mat& src_, Mat& dest_, const int templateWindowSizeX_, const int templateWindowSizeY_, const int searchWindowSizeX_, const int searchWindowSizeY_, const float* weight, const float* space_weight, const int template_maxk, const int* template_ofs, const int search_maxk, const int* search_ofs)
		: src(&src_), dest(&dest_), template_ofs(template_ofs), template_maxk(template_maxk), search_ofs(search_ofs), search_maxk(search_maxk), templateWindowSizeX(templateWindowSizeX_), templateWindowSizeY(templateWindowSizeY_), searchWindowSizeX(searchWindowSizeX_), searchWindowSizeY(searchWindowSizeY_), weight(weight), space_weight(space_weight)
	{
	}
...
/*************************************************
	using Quantization Range LUT with "gather instruction" x N
*************************************************/
class BilateralNonlocalMeansFilterInvorker_QuantizationRangeLUT_GatherxN_64f_AVX512 : public cv::ParallelLoopBody
{
private:
	const Mat * src;
	Mat* dest;
...
public:
	BilateralNonlocalMeansFilterInvorker_QuantizationRangeLUT_GatherxN_64f_AVX512(const Mat& src_, Mat& dest_, const int templateWindowSizeX_, const int templateWindowSizeY_, const int searchWindowSizeX_, const int searchWindowSizeY_, const double* weight, const double* space_weight, const int template_maxk, const int* template_ofs, const int search_maxk, const int* search_ofs)
		: src(&src_), dest(&dest_), template_ofs(template_ofs), template_maxk(template_maxk), search_maxk(search_maxk), search_ofs(search_ofs), templateWindowSizeX(templateWindowSizeX_), templateWindowSizeY(templateWindowSizeY_), searchWindowSizeX(searchWindowSizeX_), searchWindowSizeY(searchWindowSizeY_), weight(weight), space_weight(space_weight)
	{
	}
...
class BilateralNonlocalMeansFilterInvorker_QuantizationRangeLUT_GatherxN_32f_AVX512 : public cv::ParallelLoopBody
{
private:
	const Mat * src;
	Mat* dest;
...
public:
	BilateralNonlocalMeansFilterInvorker_QuantizationRangeLUT_GatherxN_32f_AVX512(const Mat& src_, Mat& dest_, const int templateWindowSizeX_, const int templateWindowSizeY_, const int searchWindowSizeX_, const int searchWindowSizeY_, const float* weight, const float* space_weight, const int template_maxk, const int* template_ofs, const int search_maxk, const int* search_ofs)
		: src(&src_), dest(&dest_), template_ofs(template_ofs), template_maxk(template_maxk), search_ofs(search_ofs), search_maxk(search_maxk), templateWindowSizeX(templateWindowSizeX_), templateWindowSizeY(templateWindowSizeY_), searchWindowSizeX(searchWindowSizeX_), searchWindowSizeY(searchWindowSizeY_), weight(weight), space_weight(space_weight)
	{
	}
...
class BilateralNonlocalMeansFilterInvorker_QuantizationRangeLUT_GatherxN_8u_AVX512 : public cv::ParallelLoopBody
{
private:
	const Mat * src;
	Mat* dest;
...
public:
	BilateralNonlocalMeansFilterInvorker_QuantizationRangeLUT_GatherxN_8u_AVX512(const Mat& src_, Mat& dest_, const int templateWindowSizeX_, const int templateWindowSizeY_, const int searchWindowSizeX_, const int searchWindowSizeY_, const float* weight, const float* space_weight, const int template_maxk, const int* template_ofs, const int search_maxk, const int* search_ofs)
		: src(&src_), dest(&dest_), template_ofs(template_ofs), template_maxk(template_maxk), search_ofs(search_ofs), search_maxk(search_maxk), templateWindowSizeX(templateWindowSizeX_), templateWindowSizeY(templateWindowSizeY_), searchWindowSizeX(searchWindowSizeX_), searchWindowSizeY(searchWindowSizeY_), weight(weight), space_weight(space_weight)
	{
	}
...
/*************************************************
	using Quantization Range LUT with "gather instruction" x 1
*************************************************/
class BilateralNonlocalMeansFilterInvorker_QuantizationRangeLUT_Gatherx1_64f_AVX512 : public cv::ParallelLoopBody
{
private:
	const Mat * src;
	Mat* dest;
...
public:
	BilateralNonlocalMeansFilterInvorker_QuantizationRangeLUT_Gatherx1_64f_AVX512(const Mat& src_, Mat& dest_, const int templateWindowSizeX_, const int templateWindowSizeY_, const int searchWindowSizeX_, const int searchWindowSizeY_, const double* weight, const double* space_weight, const int template_maxk, const int* template_ofs, const int search_maxk, const int* search_ofs)
		: src(&src_), dest(&dest_), template_ofs(template_ofs), template_maxk(template_maxk), search_maxk(search_maxk), search_ofs(search_ofs), templateWindowSizeX(templateWindowSizeX_), templateWindowSizeY(templateWindowSizeY_), searchWindowSizeX(searchWindowSizeX_), searchWindowSizeY(searchWindowSizeY_), weight(weight), space_weight(space_weight)
	{
	}
				...
				if (haveAVX512)
				{
					...
					for (; i < dest->cols; i += 8)
					{
						...
						for (int l = 0; l < search_maxk; l++, spw++)
						{
							...
							__m512i buf = _mm512_cvtpd_epi64(_mm512_maskz_rcp14_pd((__mmask8)_MM_K0_REG, _mm512_maskz_rsqrt14_pd((__mmask8)_MM_K0_REG, me)));
							const __m512d _sw = _mm512_set1_pd(*spw);
							__m512d mw = _mm512_mul_pd(_sw, _mm512_i64gather_pd(buf, w, 8));
#if __BNLMF_POSTVENTION__
							mw = _mm512_max_pd(mw, double_min);
#endif
#if __USE_FMA_INSTRUCTION__
...
class BilateralNonlocalMeansFilterInvorker_QuantizationRangeLUT_Gatherx1_32f_AVX512 : public cv::ParallelLoopBody
{
private:
	const Mat * src;
	Mat* dest;
...
public:
	BilateralNonlocalMeansFilterInvorker_QuantizationRangeLUT_Gatherx1_32f_AVX512(const Mat& src_, Mat& dest_, const int templateWindowSizeX_, const int templateWindowSizeY_, const int searchWindowSizeX_, const int searchWindowSizeY_, const float* weight, const float* space_weight, const int template_maxk, const int* template_ofs, const int search_maxk, const int* search_ofs)
		: src(&src_), dest(&dest_), template_ofs(template_ofs), template_maxk(template_maxk), search_ofs(search_ofs), search_maxk(search_maxk), templateWindowSizeX(templateWindowSizeX_), templateWindowSizeY(templateWindowSizeY_), searchWindowSizeX(searchWindowSizeX_), searchWindowSizeY(searchWindowSizeY_), weight(weight), space_weight(space_weight)
	{
	}
				...
				if (haveAVX512)
				{
					...
					for (; i < dest->cols; i += 16)
					{
						...
						for (int l = 0; l < search_maxk; l++, spw++)
						{
							...
							__m512i buf = _mm512_cvtps_epi32(_mm512_rcp14_ps(_mm512_rsqrt14_ps(me)));
							const __m512 _sw = _mm512_set1_ps(*spw);
							__m512 mw = _mm512_mul_ps(_sw, _mm512_i32gather_ps(buf, w, 4));
#if __BNLMF_POSTVENTION__
							mw = _mm512_max_ps(mw, float_min);
#endif
#if __USE_FMA_INSTRUCTION__
...
class BilateralNonlocalMeansFilterInvorker_QuantizationRangeLUT_Gatherx1_8u_AVX512 : public cv::ParallelLoopBody
{
private:
	const Mat * src;
	Mat* dest;
...
public:
	BilateralNonlocalMeansFilterInvorker_QuantizationRangeLUT_Gatherx1_8u_AVX512(const Mat& src_, Mat& dest_, const int templateWindowSizeX_, const int templateWindowSizeY_, const int searchWindowSizeX_, const int searchWindowSizeY_, const float* weight, const float* space_weight, const int template_maxk, const int* template_ofs, const int search_maxk, const int* search_ofs)
		: src(&src_), dest(&dest_), template_ofs(template_ofs), template_maxk(template_maxk), search_ofs(search_ofs), search_maxk(search_maxk), templateWindowSizeX(templateWindowSizeX_), templateWindowSizeY(templateWindowSizeY_), searchWindowSizeX(searchWindowSizeX_), searchWindowSizeY(searchWindowSizeY_), weight(weight), space_weight(space_weight)
	{
	}
				...
				if (haveAVX512)
				{
					...
					for (; i < dest->cols; i += 64)
					{
						...
						for (int l = 0; l < search_maxk; l++, spw++)
						{
							...
							//low high
							buf = _mm512_cvtps_epi32(_mm512_rcp14_ps(_mm512_rsqrt14_ps(me1)));
							mw = _mm512_mul_ps(_sw, _mm512_i32gather_ps(buf, w, 4));
#if __BNLMF_POSTVENTION__
							mw = _mm512_max_ps(mw, float_min);
#endif
							//b
...
*************************************************/
class BilateralNonlocalMeansFilterInvorker_QuantizationLUT_Gatherx1_64f_AVX512 : public cv::ParallelLoopBody
{
private:
	const Mat * src;
	Mat* dest;
...
public:
	BilateralNonlocalMeansFilterInvorker_QuantizationLUT_Gatherx1_64f_AVX512(const Mat& src_, Mat& dest_, const int templateWindowSizeX_, const int templateWindowSizeY_, const int searchWindowSizeX_, const int searchWindowSizeY_, const double h, const double sigma_space, const double* weight, const double* space_weight, const int template_maxk, const int* template_ofs, const int search_maxk, const int* search_ofs)
		: src(&src_), dest(&dest_), template_ofs(template_ofs), template_maxk(template_maxk), search_maxk(search_maxk), search_ofs(search_ofs), templateWindowSizeX(templateWindowSizeX_), templateWindowSizeY(templateWindowSizeY_), searchWindowSizeX(searchWindowSizeX_), searchWindowSizeY(searchWindowSizeY_), weight(weight), space_weight(space_weight), h(h), sigma_space(sigma_space)
	{
	}
...
class BilateralNonlocalMeansFilterInvorker_QuantizationLUT_Gatherx1_32f_AVX512 : public cv::ParallelLoopBody
{
private:
	const Mat * src;
	Mat* dest;
...
public:
	BilateralNonlocalMeansFilterInvorker_QuantizationLUT_Gatherx1_32f_AVX512(const Mat& src_, Mat& dest_, const int templateWindowSizeX_, const int templateWindowSizeY_, const int searchWindowSizeX_, const int searchWindowSizeY_, const float h, const float sigma_space, const float* weight, const float* space_weight, const int template_maxk, const int* template_ofs, const int search_maxk, const int* search_ofs)
		: src(&src_), dest(&dest_), template_ofs(template_ofs), template_maxk(template_maxk), search_ofs(search_ofs), search_maxk(search_maxk), templateWindowSizeX(templateWindowSizeX_), templateWindowSizeY(templateWindowSizeY_), searchWindowSizeX(searchWindowSizeX_), searchWindowSizeY(searchWindowSizeY_), weight(weight), space_weight(space_weight), h(h), sigma_space(sigma_space)
	{
	}
...
class BilateralNonlocalMeansFilterInvorker_QuantizationLUT_Gatherx1_8u_AVX512 : public cv::ParallelLoopBody
{
private:
	const Mat * src;
	Mat* dest;
...
public:
	BilateralNonlocalMeansFilterInvorker_QuantizationLUT_Gatherx1_8u_AVX512(const Mat& src_, Mat& dest_, const int templateWindowSizeX_, const int templateWindowSizeY_, const int searchWindowSizeX_, const int searchWindowSizeY_, const float h, const float sigma_space, const float* weight, const float* space_weight, const int template_maxk, const int* template_ofs, const int search_maxk, const int* search_ofs)
		: src(&src_), dest(&dest_), template_ofs(template_ofs), template_maxk(template_maxk), search_ofs(search_ofs), search_maxk(search_maxk), templateWindowSizeX(templateWindowSizeX_), templateWindowSizeY(templateWindowSizeY_), searchWindowSizeX(searchWindowSizeX_), searchWindowSizeY(searchWindowSizeY_), h(h), sigma_space(sigma_space), weight(weight), space_weight(space_weight)
	{
	}
...
namespace blnlmf
{
	void bilateralNonLocalMeansFilter_AVX512_64f(const Mat& src, Mat& dest, const Size templateWindowSize, const Size searchWindowSize, const double h, const double sigma_space, const int borderType, const WEIGHT_MODE weightingMethod)
	{
		...
		case WEIGHT_VECTOR_EXP:
		{
			...
			const BilateralNonlocalMeansFilterInvorker_EXP_64f_AVX512 body(im, dst, templateWindowSize.width, templateWindowSize.height, searchWindowSize.width, searchWindowSize.height, h, sigma_space, space_weight, template_maxk, template_ofs, search_maxk, search_ofs);
		...
		case WEIGHT_VECTOR_EXP_WITH_SPACE_LUT:
		{
			...
			const BilateralNonlocalMeansFilterInvorker_EXP_WITH_SPACE_LUT_64f_AVX512 body(im, dst, templateWindowSize.width, templateWindowSize.height, searchWindowSize.width, searchWindowSize.height, h, space_weight, template_maxk, template_ofs, search_maxk, search_ofs, exp_clip_val);
			cv::parallel_for_(Range(0, dst.rows), body);
			break;
		}
		case WEIGHT_RANGE_LUT_SET:
		{
			...
			const BilateralNonlocalMeansFilterInvorker_LUT_Setx3_64f_AVX512 body(im, dst, templateWindowSize.width, templateWindowSize.height, searchWindowSize.width, searchWindowSize.height, w, space_weight, template_maxk, template_ofs, search_maxk, search_ofs);
			cv::parallel_for_(Range(0, dst.rows), body);
			break;
		}
		case WEIGHT_RANGE_QUANTIZATION_LUT_SETxN:
		{
			...
			const BilateralNonlocalMeansFilterInvorker_QuantizationRangeLUT_SetxN_64f_AVX512 body(im, dst, templateWindowSize.width, templateWindowSize.height, searchWindowSize.width, searchWindowSize.height, w, space_weight, template_maxk, template_ofs, search_maxk, search_ofs);
			cv::parallel_for_(Range(0, dst.rows), body);
			break;
		}
		case WEIGHT_RANGE_QUANTIZATION_LUT_SETx1:
		{
			...
			const BilateralNonlocalMeansFilterInvorker_QuantizationRangeLUT_Setx1_64f_AVX512 body(im, dst, templateWindowSize.width, templateWindowSize.height, searchWindowSize.width, searchWindowSize.height, w, space_weight, template_maxk, template_ofs, search_maxk, search_ofs);
			cv::parallel_for_(Range(0, dst.rows), body);
			break;
		}
		case WEIGHT_MERGED_QUANTIZATION_LUT_SETx1:
		{
			...
			const BilateralNonlocalMeansFilterInvorker_QuantizationLUT_Setx1_64f_AVX512 body(im, dst, templateWindowSize.width, templateWindowSize.height, searchWindowSize.width, searchWindowSize.height, h, sigma_space, w, space_weight, template_maxk, template_ofs, search_maxk, search_ofs);
			cv::parallel_for_(Range(0, dst.rows), body);
			break;
		}
		case WEIGHT_RANGE_LUT_GATHER:
		{
			...
			const BilateralNonlocalMeansFilterInvorker_LUT_Gatherx3_64f_AVX512 body(im, dst, templateWindowSize.width, templateWindowSize.height, searchWindowSize.width, searchWindowSize.height, w, space_weight, template_maxk, template_ofs, search_maxk, search_ofs);
			cv::parallel_for_(Range(0, dst.rows), body);
			break;
		}
		case WEIGHT_RANGE_QUANTIZATION_LUT_GATHERxN:
		{
			...
			const BilateralNonlocalMeansFilterInvorker_QuantizationRangeLUT_GatherxN_64f_AVX512 body(im, dst, templateWindowSize.width, templateWindowSize.height, searchWindowSize.width, searchWindowSize.height, w, space_weight, template_maxk, template_ofs, search_maxk, search_ofs);
			cv::parallel_for_(Range(0, dst.rows), body);
			break;
		}
		case WEIGHT_RANGE_QUANTIZATION_LUT_GATHERx1:
		{
			...
			const BilateralNonlocalMeansFilterInvorker_QuantizationRangeLUT_Gatherx1_64f_AVX512 body(im, dst, templateWindowSize.width, templateWindowSize.height, searchWindowSize.width, searchWindowSize.height, w, space_weight, template_maxk, template_ofs, search_maxk, search_ofs);
			cv::parallel_for_(Range(0, dst.rows), body);
			break;
		}
		case WEIGHT_MERGED_QUANTIZATION_LUT_GATHERx1:
		{
			...
			const BilateralNonlocalMeansFilterInvorker_QuantizationLUT_Gatherx1_64f_AVX512 body(im, dst, templateWindowSize.width, templateWindowSize.height, searchWindowSize.width, searchWindowSize.height, h, sigma_space, w, space_weight, template_maxk, template_ofs, search_maxk, search_ofs);
			cv::parallel_for_(Range(0, dst.rows), body);
			break;
		}
		default:
	...
	void bilateralNonLocalMeansFilter_AVX512_32f(const Mat& src, Mat& dest, const Size templateWindowSize, const Size searchWindowSize, const float h, const float sigma_space, const int borderType, const WEIGHT_MODE weightingMethod)
	{
		if (dest.empty())dest = Mat::zeros(src.size(), src.type());
		...
		case WEIGHT_VECTOR_EXP:
		{
			int search_maxk = 0;
			setSpaceKernel(space_weight, search_ofs, search_maxk, searchH, searchV, gauss_space_coeff, im.cols, true, weightingMethod != WEIGHT_VECTOR_EXP);
			const BilateralNonlocalMeansFilterInvorker_EXP_32f_AVX512 body(im, dst, templateWindowSize.width, templateWindowSize.height, searchWindowSize.width, searchWindowSize.height, h, sigma_space, space_weight, template_maxk, template_ofs, search_maxk, search_ofs);
			cv::parallel_for_(Range(0, dst.rows), body);
			break;
		}
		case WEIGHT_VECTOR_EXP_WITH_SPACE_LUT:
		{
			...
			const BilateralNonlocalMeansFilterInvorker_EXP_WITH_SPACE_LUT_32f_AVX512 body(im, dst, templateWindowSize.width, templateWindowSize.height, searchWindowSize.width, searchWindowSize.height, h, space_weight, template_maxk, template_ofs, search_maxk, search_ofs, exp_clip_val);
			cv::parallel_for_(Range(0, dst.rows), body);
			break;
		}
		case WEIGHT_RANGE_LUT_SET:
		{
			...
			const BilateralNonlocalMeansFilterInvorker_LUT_Setx3_32f_AVX512 body(im, dst, templateWindowSize.width, templateWindowSize.height, searchWindowSize.width, searchWindowSize.height, w, space_weight, template_maxk, template_ofs, search_maxk, search_ofs);
			cv::parallel_for_(Range(0, dst.rows), body);
			break;
		}
		case WEIGHT_RANGE_QUANTIZATION_LUT_SETxN:
		{
			...
			const BilateralNonlocalMeansFilterInvorker_QuantizationRangeLUT_SetxN_32f_AVX512 body(im, dst, templateWindowSize.width, templateWindowSize.height, searchWindowSize.width, searchWindowSize.height, w, space_weight, template_maxk, template_ofs, search_maxk, search_ofs);
			cv::parallel_for_(Range(0, dst.rows), body);
			break;
		}
		case WEIGHT_RANGE_QUANTIZATION_LUT_SETx1:
		{
			...
			const BilateralNonlocalMeansFilterInvorker_QuantizationRangeLUT_Setx1_32f_AVX512 body(im, dst, templateWindowSize.width, templateWindowSize.height, searchWindowSize.width, searchWindowSize.height, w, space_weight, template_maxk, template_ofs, search_maxk, search_ofs);
			cv::parallel_for_(Range(0, dst.rows), body);
			break;
		}
		case WEIGHT_MERGED_QUANTIZATION_LUT_SETx1:
		{
			...
			const BilateralNonlocalMeansFilterInvorker_QuantizationLUT_Setx1_32f_AVX512 body(im, dst, templateWindowSize.width, templateWindowSize.height, searchWindowSize.width, searchWindowSize.height, h, sigma_space, w, space_weight, template_maxk, template_ofs, search_maxk, search_ofs);
			cv::parallel_for_(Range(0, dst.rows), body);
			break;
		}
		case WEIGHT_RANGE_LUT_GATHER:
		{
			...
			const BilateralNonlocalMeansFilterInvorker_LUT_Gatherx3_32f_AVX512 body(im, dst, templateWindowSize.width, templateWindowSize.height, searchWindowSize.width, searchWindowSize.height, w, space_weight, template_maxk, template_ofs, search_maxk, search_ofs);
			cv::parallel_for_(Range(0, dst.rows), body);
			break;
		}
		case WEIGHT_RANGE_QUANTIZATION_LUT_GATHERxN:
		{
			...
			const BilateralNonlocalMeansFilterInvorker_QuantizationRangeLUT_GatherxN_32f_AVX512 body(im, dst, templateWindowSize.width, templateWindowSize.height, searchWindowSize.width, searchWindowSize.height, w, space_weight, template_maxk, template_ofs, search_maxk, search_ofs);
			cv::parallel_for_(Range(0, dst.rows), body);
			break;
		}
		case WEIGHT_RANGE_QUANTIZATION_LUT_GATHERx1:
		{
			...
			const BilateralNonlocalMeansFilterInvorker_QuantizationRangeLUT_Gatherx1_32f_AVX512 body(im, dst, templateWindowSize.width, templateWindowSize.height, searchWindowSize.width, searchWindowSize.height, w, space_weight, template_maxk, template_ofs, search_maxk, search_ofs);
			cv::parallel_for_(Range(0, dst.rows), body);
			break;
		}
		case WEIGHT_MERGED_QUANTIZATION_LUT_GATHERx1:
		{
			...
			const BilateralNonlocalMeansFilterInvorker_QuantizationLUT_Gatherx1_32f_AVX512 body(im, dst, templateWindowSize.width, templateWindowSize.height, searchWindowSize.width, searchWindowSize.height, h, sigma_space, w, space_weight, template_maxk, template_ofs, search_maxk, search_ofs);
			cv::parallel_for_(Range(0, dst.rows), body);
			break;
		}
		default:
	...
	void bilateralNonLocalMeansFilter_AVX512_8u(const Mat& src, Mat& dest, const Size templateWindowSize, const Size searchWindowSize, const float h, const float sigma_space, const int borderType, const WEIGHT_MODE weightingMethod)
	{
		if (dest.empty())dest = Mat::zeros(src.size(), src.type());
		...
		case WEIGHT_VECTOR_EXP:
		{
			int search_maxk = 0;
			setSpaceKernel(space_weight, search_ofs, search_maxk, searchH, searchV, gauss_space_coeff, im.cols, true, weightingMethod != WEIGHT_VECTOR_EXP);
			const BilateralNonlocalMeansFilterInvorker_EXP_8u_AVX512 body(im, dst, templateWindowSize.width, templateWindowSize.height, searchWindowSize.width, searchWindowSize.height, h, sigma_space, space_weight, template_maxk, template_ofs, search_maxk, search_ofs);
			cv::parallel_for_(Range(0, dst.rows), body);
			break;
		}
		case WEIGHT_VECTOR_EXP_WITH_SPACE_LUT:
		{
			...
			const BilateralNonlocalMeansFilterInvorker_EXP_WITH_SPACE_LUT_8u_AVX512 body(im, dst, templateWindowSize.width, templateWindowSize.height, searchWindowSize.width, searchWindowSize.height, h, space_weight, template_maxk, template_ofs, search_maxk, search_ofs, exp_clip_val);
			cv::parallel_for_(Range(0, dst.rows), body);
			break;
		}
		case WEIGHT_RANGE_LUT_SET:
		{
			...
			const BilateralNonlocalMeansFilterInvorker_LUT_Setx3_8u_AVX512 body(im, dst, templateWindowSize.width, templateWindowSize.height, searchWindowSize.width, searchWindowSize.height, w, space_weight, template_maxk, template_ofs, search_maxk, search_ofs);
			cv::parallel_for_(Range(0, dst.rows), body);
			break;
		}
		case WEIGHT_RANGE_QUANTIZATION_LUT_SETxN:
		{
			...
			const BilateralNonlocalMeansFilterInvorker_QuantizationRangeLUT_SetxN_8u_AVX512 body(im, dst, templateWindowSize.width, templateWindowSize.height, searchWindowSize.width, searchWindowSize.height, w, space_weight, template_maxk, template_ofs, search_maxk, search_ofs);
			cv::parallel_for_(Range(0, dst.rows), body);
			break;
		}
		case WEIGHT_RANGE_QUANTIZATION_LUT_SETx1:
		{
			...
			const BilateralNonlocalMeansFilterInvorker_QuantizationRangeLUT_Setx1_8u_AVX512 body(im, dst, templateWindowSize.width, templateWindowSize.height, searchWindowSize.width, searchWindowSize.height, w, space_weight, template_maxk, template_ofs, search_maxk, search_ofs);
			cv::parallel_for_(Range(0, dst.rows), body);
			break;
		}
		case WEIGHT_MERGED_QUANTIZATION_LUT_SETx1:
		{
			...
			const BilateralNonlocalMeansFilterInvorker_QuantizationLUT_Setx1_8u_AVX512 body(im, dst, templateWindowSize.width, templateWindowSize.height, searchWindowSize.width, searchWindowSize.height, h, sigma_space, w, space_weight, template_maxk, template_ofs, search_maxk, search_ofs);
			cv::parallel_for_(Range(0, dst.rows), body);
			break;
		}
		case WEIGHT_RANGE_LUT_GATHER:
		{
			...
			const BilateralNonlocalMeansFilterInvorker_LUT_Gatherx3_8u_AVX512 body(im, dst, templateWindowSize.width, templateWindowSize.height, searchWindowSize.width, searchWindowSize.height, w, space_weight, template_maxk, template_ofs, search_maxk, search_ofs);
			cv::parallel_for_(Range(0, dst.rows), body);
			break;
		}
		case WEIGHT_RANGE_QUANTIZATION_LUT_GATHERxN:
		{
			...
			const BilateralNonlocalMeansFilterInvorker_QuantizationRangeLUT_GatherxN_8u_AVX512 body(im, dst, templateWindowSize.width, templateWindowSize.height, searchWindowSize.width, searchWindowSize.height, w, space_weight, template_maxk, template_ofs, search_maxk, search_ofs);
			cv::parallel_for_(Range(0, dst.rows), body);
			break;
		}
		case WEIGHT_RANGE_QUANTIZATION_LUT_GATHERx1:
		{
			...
			const BilateralNonlocalMeansFilterInvorker_QuantizationRangeLUT_Gatherx1_8u_AVX512 body(im, dst, templateWindowSize.width, templateWindowSize.height, searchWindowSize.width, searchWindowSize.height, w, space_weight, template_maxk, template_ofs, search_maxk, search_ofs);
			cv::parallel_for_(Range(0, dst.rows), body);
			break;
		}
		case WEIGHT_MERGED_QUANTIZATION_LUT_GATHERx1:
		{
			...
			const BilateralNonlocalMeansFilterInvorker_QuantizationLUT_Gatherx1_8u_AVX512 body(im, dst, templateWindowSize.width, templateWindowSize.height, searchWindowSize.width, searchWindowSize.height, h, sigma_space, w, space_weight, template_maxk, template_ofs, search_maxk, search_ofs);
			cv::parallel_for_(Range(0, dst.rows), body);
			break;
		}
		default:
	...
	void bilateralNonLocalMeansFilter_AVX512(InputArray src_, OutputArray dest, const Size templateWindowSize, const Size searchWindowSize, const double h, const double sigma_space, const int borderType, const WEIGHT_MODE weightingMethod)
	{
		...
		case CV_8U:
		{
			bilateralNonLocalMeansFilter_AVX512_8u(src, dst, templateWindowSize, searchWindowSize, (float)h, sigma_space, borderType, weightingMethod);
			break;
		}
		case CV_32F:
		{
			bilateralNonLocalMeansFilter_AVX512_32f(src, dst, templateWindowSize, searchWindowSize, (float)h, sigma_space, borderType, weightingMethod);
			break;
		}
		case CV_64F:
		{
			bilateralNonLocalMeansFilter_AVX512_64f(src, dst, templateWindowSize, searchWindowSize, h, sigma_space, borderType, weightingMethod);
			break;
		}
		}
	}
