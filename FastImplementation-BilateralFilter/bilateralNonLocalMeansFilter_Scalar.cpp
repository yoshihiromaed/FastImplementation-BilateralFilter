...
class BilateralNonlocalMeansFilterInvorker_64f : public cv::ParallelLoopBody
{
private:
	const Mat * src;
	Mat* dest;
...
class BilateralNonlocalMeansFilterInvorker_32f : public cv::ParallelLoopBody
{
private:
	const Mat * src;
	Mat* dest;
...
class BilateralNonlocalMeansFilterInvorker_8u : public cv::ParallelLoopBody
{
private:
	const Mat * src;
	Mat* dest;
...
public:
	BilateralNonlocalMeansFilterInvorker_8u(const Mat& src_, Mat& dest_, const int templateWindowSizeX_, const int templateWindowSizeY_, const int searchWindowSizeX_, const int searchWindowSizeY_, const float h, const float sigma_space, const float* space_weight, const int template_maxk, const int* template_ofs, const int search_maxk, const int* search_ofs)
		: src(&src_), dest(&dest_), template_ofs(template_ofs), template_maxk(template_maxk), search_ofs(search_ofs), search_maxk(search_maxk), templateWindowSizeX(templateWindowSizeX_), templateWindowSizeY(templateWindowSizeY_), searchWindowSizeX(searchWindowSizeX_), searchWindowSizeY(searchWindowSizeY_), h(h), sigma_space(sigma_space), space_weight(space_weight)
	{
	}
...
namespace blnlmf
{
	void bilateralNonLocalMeansFilter_Scalar(const Mat& src, Mat& dest, const Size templateWindowSize, const Size searchWindowSize, const double h, const double sigma_space, const int borderType, const bool isParallel)
	{
		...
		if (src.depth() == CV_8U)
		{
			...
			const BilateralNonlocalMeansFilterInvorker_8u body(im, dest, templateWindowSize.width, templateWindowSize.height, searchWindowSize.width, searchWindowSize.height, h, sigma_space, space_weight, template_maxk, template_ofs, search_maxk, search_ofs);
		...
		else if (src.depth() == CV_32F)
		{
			...
			const BilateralNonlocalMeansFilterInvorker_32f body(im, dest, templateWindowSize.width, templateWindowSize.height, searchWindowSize.width, searchWindowSize.height, h, sigma_space, space_weight, template_maxk, template_ofs, search_maxk, search_ofs);
			cv::parallel_for_(Range(0, dest.rows), body, isParallel ? -1 : 1);
		}
		else if (src.depth() == CV_64F)
		{
			...
			const BilateralNonlocalMeansFilterInvorker_64f body(im, dest, templateWindowSize.width, templateWindowSize.height, searchWindowSize.width, searchWindowSize.height, h, sigma_space, space_weight, template_maxk, template_ofs, search_maxk, search_ofs);
			cv::parallel_for_(Range(0, dest.rows), body, isParallel ? -1 : 1);
		}
	}
}