...
namespace fmath {
	...
	namespace local {
		...
		*/
		...
		MIE_ALIGN(64) const ExpVar<EXP_N> C<EXP_N, LOG_N, EXPD_N>::expVar;
	...
	*/
	...
	inline __m256 exp_ps256(__m256 x)
	{
		...
		__m256 limit = _mm256_and_ps(x, *(const __m256*)expVar.i7fffffff);
		int over = _mm256_movemask_ps(_mm256_cmp_ps(limit, *(const __m256*)expVar.maxX, _CMP_GT_OQ));
		...
		__m256i rr = _mm256_cvtps_epi32(_mm256_mul_ps(x, *(const __m256*)expVar.a));
		__m256 t = _mm256_sub_ps(x, _mm256_mul_ps(_mm256_cvtepi32_ps(rr), *(const __m256*)expVar.b));
		t = _mm256_add_ps(t, *(const __m256*)expVar.f1);

		__m256 r = _mm256_castsi256_ps(rr);
		__m256 vv8 = _mm256_and_ps(r, *(const __m256*)expVar.mask_s);
		__m256i v8 = _mm256_castps_si256(vv8);


		__m256i u8 = _mm256_setzero_si256();
		__m128i* u8L = (__m128i*)((int *)&u8 + 0);
		__m128i* u8H = (__m128i*)((int *)&u8 + 4);

		*u8L = _mm_add_epi32(*(const __m128i*)((int *)&rr + 0), *(const __m128i*)expVar.i127s);
		*u8H = _mm_add_epi32(*(const __m128i*)((int *)&rr + 4), *(const __m128i*)expVar.i127s);
		*u8L = _mm_srli_epi32(*u8L, expVar.s);
		*u8L = _mm_slli_epi32(*u8L, 23);
		*u8H = _mm_srli_epi32(*u8H, expVar.s);
		*u8H = _mm_slli_epi32(*u8H, 23);

		unsigned int v0, v1, v2, v3, v4, v5, v6, v7;
		//v0 = _mm256_extract_epi32(v8, 0);
		//v1 = _mm256_extract_epi32(v8, 1);
		//v2 = _mm256_extract_epi32(v8, 2);
		//v3 = _mm256_extract_epi32(v8, 3);
		//v4 = _mm256_extract_epi32(v8, 4);
		//v5 = _mm256_extract_epi32(v8, 5);
		//v6 = _mm256_extract_epi32(v8, 6);
		//v7 = _mm256_extract_epi32(v8, 7);
		v0 = v8.m256i_u32[0];
		v1 = v8.m256i_u32[1];
		v2 = v8.m256i_u32[2];
		v3 = v8.m256i_u32[3];
	...
	inline __m512 exp_ps512(__m512 x)
	{
		...
		__m512i limit = _mm512_castps_si512(_mm512_and_ps(x, *(const __m512*)expVar.i7fffffff));
		int over = (_mm512_cmpgt_epi32_mask(limit, *(const __m512i*)expVar.maxX));
		if (over) {
			x = _mm512_min_ps(x, _mm512_load_ps(expVar.maxX));
			x = _mm512_max_ps(x, _mm512_load_ps(expVar.minX));
		}
		__m512i r = _mm512_cvtps_epi32(_mm512_mul_ps(x, *(const __m512*)expVar.a));
		__m512 t = _mm512_sub_ps(x, _mm512_mul_ps(_mm512_cvtepi32_ps(r), *(const __m512*)expVar.b));
		t = _mm512_add_ps(t, *(const __m512*)expVar.f1);
		__m512i v8 = _mm512_and_si512(r, *(const __m512i*)expVar.mask_s);
		__m512i u8 = _mm512_add_epi32(r, *(const __m512i*)expVar.i127s);
